<template>
  <div>
    <div v-for="(network, index) in networks" :key="index" class="relative">
      <div
        :class="{
      ...
      >
        ...
        <div :class="{'flex my-auto w-6': true, hidden: disabled || network.accounts.length < 1 }">
          <ChevronUpIcon v-if="selectedNetworkIndex === index && selectedNetworkActive === true" />
          <ChevronDownIcon v-else />
        </div>
        <div :class="{'flex my-auto': true, hidden: disabled || network.accounts.length> 0 }">
          <button
      ...
      >
          <li v-for="account in network.accounts" :key="account.address">
            ...
            <div className="flex justify-between w-full active:text-gray-800 hover:bg-transparent focus:bg-transparent active:bg-gray-300 active:bg-transparent cursor-default">
              ...
              <div className="px-2 text-right cursor-pointer" @click="onDisconnectAccount(account)" title="Logout">
