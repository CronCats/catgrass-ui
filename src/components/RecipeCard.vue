      ...
      <div class="flex justify-between mt-2">
        ...
        <div class="flex">
          <div v-for="(chain, index) in recipeChains" :key="index" :class="{ 'ml-[-12px]' : index> 0}">
            <LogoFromImage class="block" :rounded="true" size="32" :src="chain?.asset?.logo_URIs?.png || ''" />
          </div>
        </div>
      </div>
      <slot name="body"></slot>
    </div>
    <div v-if="data.recipeHash" class="relative z-10 px-6 pt-5 pb-2 -mt-4 bg-gray-200 rounded-b-2xl">
      <div class="flex justify-between text-xs">
        <small class="overflow-hidden pr-8 my-auto w-full text-ellipsis break-normal">
          Hash:&nbsp;
          <span class="overflow-hidden text-ellipsis break-normal">
            {{data.recipeHash}}
          </span>
        </small>
        <DocumentDuplicateIcon class="w-5 h-5 text-gray-500 hover:text-gray-700 cursor-pointer" />
      </div>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  computed: {
    recipeChains() {
      ...
      return n.filter(function (item, pos, self) {
