<template>
  <Listbox :onChange="updateSelect"  v-model="selected">
    <div class="relative mt-1">
      ...
      <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <ListboxOptions
          class="overflow-auto absolute z-10 py-1 mt-0 w-full max-h-56 text-base bg-white rounded-md focus:outline-none ring-2 ring-gray-200 shadow-lg sm:text-sm">
          <ListboxOptions v-for="(item, index) in options" v-slot="{ active, selected }" :key="index" :class="[
          ...
          >
            <div class="flex items-center">
              <div v-if="item.value.Icon" class="flex-shrink-0 w-8 h-8">
                <component :is="item.value.Icon" />
              </div>
              <span class="block ml-3 text-lg font-bold truncate">
                {{item.value.title}}
              </span>
...
export default {
  props: ["options", "updateSelect", "containerClassName", "className"],
