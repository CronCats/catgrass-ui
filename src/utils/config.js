...
const stargaze = chains.filter(c => c.chain_id == 'stargaze-1')[0]
const juno = chains.filter(c => c.chain_id == 'juno-1')[0]
...
class ConfigManager {
  ...
  formatConfig = (raw) => {
    let contractAddress = "";
    // if (raw.network_type === "testnet")
    //   contractAddress = deploysTestnet[raw.chain_id];
    // if (raw.network_type === "mainnet")
    //   contractAddress = deploysMainnet[raw.chain_id];
